...
begin
  ...
  if next_pinned then
    ...
    if pinned_count>=3 then
      raise exception using errcode='22023', message='PIN_LIMIT_REACHED';
    end if;
  end if;

  insert into public.conversation_user_state(
    conversation_id,user_id,pinned_at,updated_at
  )
  values(
    cid,
    uid,
    case when next_pinned then now() else null end,
    now()
  )
  on conflict on constraint conversation_user_state_pkey
  do update set pinned_at=excluded.pinned_at,updated_at=now();
end;
$set_dm_pinned$;

revoke execute on function public.set_conversation_pinned(uuid,boolean)
from public,anon;
grant execute on function public.set_conversation_pinned(uuid,boolean)
to authenticated;

create or replace function public.get_dm_inbox_v4(
...
as $dm_inbox_v4$
  ...
  order by
    state.pinned_at desc nulls last,
    coalesce(lm.created_at,c.last_message_at,c.created_at) desc;
$dm_inbox_v4$;

revoke execute on function public.get_dm_inbox_v4(boolean)
from public,anon;
grant execute on function public.get_dm_inbox_v4(boolean)
to authenticated;
