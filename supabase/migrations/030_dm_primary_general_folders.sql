alter table public.conversation_user_state
  add column if not exists inbox_folder text not null default 'primary';

do $folder_check$
begin
  alter table public.conversation_user_state
    add constraint conversation_user_state_inbox_folder_check
    check (inbox_folder in ('primary','general'));
...
begin
  ...
  if next_folder not in ('primary','general') then
  ...
  insert into public.conversation_user_state(
    conversation_id,user_id,inbox_folder,updated_at
  )
  values(cid,uid,next_folder,now())
  on conflict on constraint conversation_user_state_pkey
  do update set inbox_folder=excluded.inbox_folder,updated_at=now();
end;
$set_dm_folder$;

revoke execute on function public.set_conversation_folder(uuid,text)
from public,anon;
grant execute on function public.set_conversation_folder(uuid,text)
to authenticated;

create or replace function public.get_dm_inbox_v3(
  include_requests boolean default false
)
returns table(
...
as $dm_inbox_v3$
  ...
  select
    ...
    coalesce(state.inbox_folder,'primary')
  from mine
...
revoke execute on function public.get_dm_inbox_v3(boolean)
from public,anon;
grant execute on function public.get_dm_inbox_v3(boolean)
to authenticated;
